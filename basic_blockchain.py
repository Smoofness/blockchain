...
class Blockchain:
    ...
    def create_block(self, proof, previous_hash):
        sender = self.get_last_sender()
        recipient = random.choice(people)
        while sender == recipient:
            recipient = random.choice(people)
        transaction = {"amount": random.randint(1, 100), "recipient": recipient, "sender": sender}
    ...
    def get_last_sender(self):
        ...
        if len(transactions) > 0:
            sneder = transactions[-1]['recipient']
            return sneder
        else:
            return "Bob"
    ...
    #tried to make graph of times, issues with pandas. 
        new_proof = 1
        check_proof = False
        times = []
